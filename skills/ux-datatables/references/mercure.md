...
Three more API Platform specifics: `@=` expression topics are skipped by the resolver (only plain string topics are reused verbatim, so `@=iri(object.getOwner())` silently falls back to the item route path), `private: true` is mapped to `withCredentials` automatically (see Private topics above), and the subscriber cookie comes from symfony/mercure-bundle (`Authorization`, the `mercure()` Twig function), not from this bundle — and `@=` needs `symfony/expression-language` installed on the app side or API Platform throws when it publishes.
