export function isUrlStateEnabled(payload) {
    const cfg = payload?.urlState;
    if (!cfg || typeof cfg !== 'object')
        return null;
    if (!cfg.search && !cfg.order && !cfg.page && !cfg.pageLength)
        return null;
    return {
        search: !!cfg.search,
        order: !!cfg.order,
...
function paramName(cfg, key) {
    return cfg.prefix ? `${cfg.prefix}[${key}]` : key;
}
function orderBase(cfg) {
    return cfg.prefix ? `${cfg.prefix}[order]` : 'order';
}
function clearOrderParams(params, base) {
    const toDelete = [];
...
function resolveColumnName(table, idx) {
    try {
        return table.column(idx).name();
    }
    catch {
        return String(idx);
    }
}
function buildOrderEntries(table) {
    const raw = table.order();
    if (!raw || raw.length === 0)
        return [];
    return raw.map((o) => {
        if (Array.isArray(o)) {
            return { name: resolveColumnName(table, o[0]), dir: o[1] };
        }
        if (o.name)
            return { name: o.name, dir: o.dir };
        return {
...
export function readUrlState(cfg, search = window.location.search) {
...
export function applyUrlStateToPayload(payload, snapshot) {
    ...
    if (snapshot.start !== undefined)
        payload.displayStart = snapshot.start;
    if (snapshot.pageLength !== undefined)
        payload.pageLength = snapshot.pageLength;
}
export function writeUrlState(cfg, table) {
