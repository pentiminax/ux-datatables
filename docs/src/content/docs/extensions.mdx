---
title: Extensions Overview
description: Extension hub with prerequisites and selection guidance
order: 0
---

UX DataTables supports common DataTables extensions through PHP helpers and lazy-loaded frontend adapters.

## Prerequisites

- Add extension configuration on your `DataTable` instance.
- Keep extension combinations incremental and tested.
- Prefer explicit setup per table when behavior differs between screens.

## Extension Matrix

<Table
  headers={['Extension', 'Main use case', 'Doc']}
  rows={[
    ['Buttons', 'Export and utility actions', '/ux-datatables/extensions/buttons/'],
    ['Select', 'Row/column/cell selection', '/ux-datatables/extensions/select/'],
    [
      'Column Control',
      'Per-column order/search controls',
      '/ux-datatables/extensions/column-control/',
    ],
    ['Responsive', 'Mobile-friendly tables', '/ux-datatables/extensions/responsive/'],
    [
      'Row Group',
      'Group related rows with optional summaries',
      '/ux-datatables/extensions/row-group/',
    ],
    ['KeyTable', 'Keyboard navigation', '/ux-datatables/extensions/keytable/'],
    ['Scroller', 'Virtualized rendering', '/ux-datatables/extensions/scroller/'],
    [
      'Fixed Columns',
      'Keep edge columns visible while scrolling',
      '/ux-datatables/extensions/fixed-columns/',
    ],
    [
      'Fixed Header',
      'Keep the header visible while scrolling the page',
      '/ux-datatables/extensions/fixed-header/',
    ],
    ['ColReorder', 'User-driven column order', '/ux-datatables/extensions/col-reorder/'],
  ]}
/>

## Combining Extensions

Use the dedicated page for tested patterns and caveats:

- [/ux-datatables/extensions/combining-extensions/](/ux-datatables/extensions/combining-extensions/)
