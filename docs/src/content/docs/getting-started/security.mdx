...
<Aside type="caution" title="Check unanimous access decisions">
  If your application voters deny attributes used by `DataTable::setPermission()` or
  `Action::permission()`, configure Symfony's `unanimous` strategy. Symfony's default
  `affirmative` strategy can still grant access when the bundle voter grants and your voter denies.
</Aside>
