...
<Table
  ...
  rows={[
    ...
    [
      ...
      'POST',
      'Template column rendering (collection and item security applied by API Platform)',
    ],
    ['ux_datatables_ajax_export', '/datatables/ajax/export', 'POST', 'Server-side CSV/XLSX export'],
  ]}
/>

`/datatables/ajax/templates` receives a collection query, not rows. It runs that query through an
API Platform GET subrequest, so collection providers, Doctrine extensions and operation security
decide which entities may be rendered. It then evaluates item `Get` security in memory before
rendering templates, actions or URLs. See
