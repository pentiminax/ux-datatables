...
server-side exports, the inline edit modal, row deletion, detail rendering and bulk actions all go
...
<Table
  ...
  rows={[
    ...
    ['ux_datatables_ajax_bulk', '/datatables/ajax/bulk', 'POST', 'Bulk actions over selected rows'],
    ['ux_datatables_ajax_export', '/datatables/ajax/export', 'POST', 'Server-side CSV/XLSX export'],
  ]}
/>

Every one of these routes relies entirely on the `access_control` rule below.

A table that combines API Platform with a `TemplateColumn`, an action column or a `UrlColumn` reads
its rows through `ux_datatables_ajax_data` rather than querying the API from the browser. The
collection operation's `security` expression still runs on every draw, but the rows are read through
the API Platform state provider, not through an HTTP request: `access_control` rules matching the
API path are **not** evaluated for them. Secure such a table exactly as you would a Doctrine table —
with the rule below, plus `security` on the collection operation. See
[Reading the collection server-side](/integrations/api-platform/#reading-the-collection-server-side).
...
5. `security` on the collection operation of every API Platform table that renders template, action
   or URL columns — the API path's own `access_control` rule does not cover it.
6. Unauthenticated probe of `/datatables/ajax/data` returns `401`/`403`, not table JSON.
