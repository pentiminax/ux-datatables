...
Permission::DT_EXECUTE_ACTION;     // Action::permission() checks
Permission::DT_EDIT_ROW;           // edit modal submit and boolean toggle writes
Permission::DT_DELETE_ROW;         // delete mutation
Permission::DT_VIEW_ROW_DETAILS;   // collapsible detail-row fallback
```

`Permission::DT_ACCESS_TABLE` and `Permission::DT_EXECUTE_ACTION` are handled by the bundle voter.
That voter delegates `DataTable::setPermission()` and `Action::permission()` to Symfony's access
decision manager with the current token. Your application voters still own the permissions you pass
to those methods.

<Aside type="caution" title="Check unanimous access decisions">
  If your application uses Symfony's `unanimous` access decision strategy, every voter that votes on
  the delegated attribute must grant access. A single deny wins. Make sure catch-all voters abstain
  for attributes they do not own, and test table rendering plus Ajax requests after enabling
  `DataTable::setPermission()` or `Action::permission()`.
</Aside>
...
- collapsible detail rows always check `isGranted(Permission::DT_VIEW_ROW_DETAILS, $entity)`.
  ...
  has `Action::permission()`, that permission must grant too.
...
Custom actions keep their own route authorization and CSRF boundary. `Action::permission()` only
