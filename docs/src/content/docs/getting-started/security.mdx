...
That voter delegates `DataTable::setPermission()` and `Action::setPermission()` to Symfony's access
...
<Aside type="caution" title="Check unanimous access decisions">
  ...
  `Action::setPermission()`, configure Symfony's `unanimous` strategy. Symfony's default
