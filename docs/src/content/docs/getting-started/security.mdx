...
Route protection is the first boundary. The bundle also calls Symfony voters for table rendering,
table Ajax access, row actions, and built-in row mutations.
...
(`SecurityVoter`). It only supports those two attributes and abstains on everything else, so it never
...
bundle calls Symfony authorization before reading private row details or mutating anything:
...
- collapsible detail rows always check `isGranted(Permission::DT_VIEW_ROW_DETAILS, $entity)`.
  The configured detail action also passes through `Permission::DT_EXECUTE_ACTION`; if the action
  has `Action::setPermission()`, that permission must grant too.
...
The check is performed through a small internal wrapper, `AuthorizationChecker`, which only grants
...
3. Voters for `Permission::DT_EDIT_ROW`, `Permission::DT_DELETE_ROW`, and
