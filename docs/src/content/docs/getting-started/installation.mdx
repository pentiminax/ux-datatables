...
## DataTables 3 Baseline

UX DataTables targets DataTables.net core 3 and its matching extension packages. DataTables 1.x and
2.x are no longer supported by the bundle runtime.

When upgrading an application to the DataTables 3 baseline, review any custom JavaScript that reaches
into the DataTables API directly:

- callback `this` values are no longer jQuery objects
- selector arguments should use native selectors
- `columns().search()` uses the DataTables 3 search semantics
- custom styling integrations should read `settings.classes` instead of `settings.oClasses` and
  call `DataTable.ext.rendererDisplayRowCells()` instead of the removed private
  `DataTable.ext.renderer.layout._forLayoutRow()` helper
- legacy cache APIs and `scrollXInner` are removed
- SCSS variable overrides must move to CSS-level theming or application styles

The legacy `dom`, `pagingType`, and `orderCellsTop` options remain accepted for compatibility.
