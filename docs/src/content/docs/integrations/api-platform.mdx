...
The route therefore rehydrates each row through the resource's API Platform `Get` operation, using
the row's `@id` when present and its `id` otherwise. That applies exactly what a
`GET /resource/{id}` would: the operation's state provider, its Doctrine item extensions
(`QueryItemExtensionInterface`, tenant scoping, filtered query builders) and its `security`
expression.
