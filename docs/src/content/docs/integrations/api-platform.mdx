...
converter does; a row carrying only an `id` generates an IRI through the resource's first `Get`
before it is loaded. This lets API Platform transform custom identifiers such as UUID or value
objects before invoking the provider. Both forms apply exactly what a `GET /resource/{id}` would:
the operation's state provider, its Doctrine item
