...
sends the collection query and DataTables draw number to `POST /datatables/ajax/templates`. It does
not send rows or identifiers.

The route resolves the collection URL from API Platform metadata and runs one GET subrequest with
the same query string. This goes through API Platform's normal collection provider, filters,
pagination, eager loading and operation security. The route reads the loaded entities from the
subrequest and maps the page through the normal row pipeline, so template, action and URL rendering
does not perform one item lookup per row.

The browser still fetches the Hydra collection first in this compatibility path. Template-enabled
tables therefore make two collection requests, but the server performs no N+1 item hydration.

When the resource has a `Get` operation, its item `security` expression is also evaluated against
each entity already loaded by the collection request. This check performs no item query. A denied
item keeps its serialized collection row but receives no template, action or URL output.
...
<Aside type="caution" title="A resource with no `Get` operation renders nothing">
  Without a `Get` operation, item security cannot be evaluated, so every row comes back unrendered.
  The route caps `itemsPerPage` at `data_tables.max_page_length` (1000 by default) and rejects a
  collection that still returns more rows than that limit.
</Aside>
