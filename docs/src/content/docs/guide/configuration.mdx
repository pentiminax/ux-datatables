...
A page size the table declares itself raises the bound instead of being capped by it, because the
client paginates with that size: `pageLength()` and the entries of `lengthMenu()` are honored even
above `max_page_length`. The bound still applies to any other `length` a request carries, and a
refused "show all" still falls back to `max_page_length`.
