...
<Aside type="caution" title="Detail rows always require DT_VIEW_ROW_DETAILS">
  `Permission::DT_VIEW_ROW_DETAILS` is required on top of any
  `Action::detail()->setPermission(...)` you declare, with no fallback. Without a voter supporting
  `DT_VIEW_ROW_DETAILS` on the entity, every detail request responds `403`. Register one (see
  [Writing a voter](#writing-a-voter) below).
</Aside>
