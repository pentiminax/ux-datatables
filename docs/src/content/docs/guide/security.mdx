...
- `delete()` checks `DT_DELETE_ROW` on the entity before removing it.
- `setProperty()` (the inline boolean toggle) checks `DT_EDIT_ROW` on the entity before writing
  the field.
- Collapsible detail rows always check `DT_VIEW_ROW_DETAILS` on the entity.
