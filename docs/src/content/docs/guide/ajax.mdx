...
<Aside type="caution">
  This first version targets client-side DataTables loading. Use a collection endpoint returning the full dataset (not paginated partial pages).
</Aside>

<Aside type="tip">
  For complex server-side tables, consider using [AbstractDataTable](../reference/abstract-datatable/) with the `#[AsDataTable]` attribute for automatic Doctrine integration.
</Aside>
