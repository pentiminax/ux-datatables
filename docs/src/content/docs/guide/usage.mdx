...
Table classes are autoconfigured services. Inject one into a controller and pass it to the
template:
...
and call [`setData()`](/ux-datatables/reference/abstract-datatable/) on the injected table instead.
...
<Table
  ...
  rows={[
    ...
    ['datatables:bulk:start', 'Fired when a bulk action is about to be sent, after any confirmation', 'action: action name, selection: selected ids, allMatching, deselectedIds and counts'],
    ['datatables:bulk:success', 'Fired when the bulk endpoint accepted the run', 'action: action name, result: success, processed, skipped, message'],
    ['datatables:bulk:error', 'Fired when the endpoint refused the run or the request failed', 'action: action name, and result (refused) or error (network failure)'],
  ]}
/>

The `bulk:*` events only fire on a table that declares
[bulk actions](/ux-datatables/features/bulk-actions/).
...
[Custom Content Types](/ux-datatables/extensions/column-control/#custom-content-types).
...
<Aside type="caution">
  Each table must have a unique ID. The ID is the short name of the table class, so two tables
  rendered on the same page must not share a class name.
</Aside>
