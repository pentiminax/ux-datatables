...
Inject the table into a controller and render it:
...
When the rows are computed in the controller, call `$table->setData($rows)` there instead of
hardcoding them in `configureDataTable()`.
