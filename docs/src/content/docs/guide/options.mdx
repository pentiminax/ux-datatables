...
1. **Display** — width, caption, initial order
2. **Ajax & data** — sources and request shaping
3. **Features** — info, paging, search, ordering toggles
4. **Scrolling / pagination / search / i18n / layout** — detailed knobs
...
Saves: page position, ordering, search term, page length, and the values applied in the
[filter bar](/ux-datatables/reference/filters/#remembering-applied-filters).
...
Declare which CSS framework the table renders against, instead of relying on the frontend to
detect it from the page's stylesheets:
