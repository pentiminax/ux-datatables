...
<Aside type="caution">
On MySQL and SQLite, where UUIDs are usually stored as text, partial search on these columns is not
supported. Add a searchable text column (a slug, a reference number, or a `string` copy of the
identifier) when users need partial matching.
</Aside>
