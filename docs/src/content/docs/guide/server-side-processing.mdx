...
### PostgreSQL and typed fields (uuid, guid)
...
before applying `LOWER(… ) LIKE ?`. This resolves the common runtime error:

```
operator does not exist: uuid ~~ text
```

…that occurs when a column is mapped with Doctrine type `uuid` or `guid` and a
LIKE search is applied without an explicit cast. No configuration is required;
the function is registered automatically when DoctrineBundle is present.

The cast is a no-op for ordinary `string` and `text` fields in PostgreSQL, so
there is no performance difference for those columns.
