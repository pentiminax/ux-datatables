...
If your table uses `#[AsDataTable(Entity::class)]`, `entity` is injected automatically into the `BooleanColumn`.
...
For image fields rendered as `<img>` thumbnails. Lazy loading is enabled by default. See the [ImageColumn reference](../../reference/image-column/) for all options.
...
For link cells, use `UrlColumn`. It can render a raw URL value or resolve the `href` from a Symfony
route or a per-row callable. See the [UrlColumn reference](../../reference/url-column/) for the full API.
...
Exclude a column from the [inline edit modal](/ux-datatables/reference/action-columns/#inline-edit-modal):
...
<Table
  ...
  rows={[
    ['DATE', 'Date values'],
    ['HTML', 'HTML content'],
    ['HTML_NUM', 'Numeric values extracted from HTML content'],
    ['HTML_NUM_FMT', 'Formatted numeric values extracted from HTML content'],
    ['HTML_UTF8', 'HTML content with UTF-8 aware text sorting'],
    ['NUM', 'Numeric values (also used by BooleanColumn for sorting)'],
...
<Aside type="tip">
  For row actions, use `AbstractDataTable::configureActions()` or
  `ActionColumn::fromActions()`. See the [Action Columns](../../reference/action-columns/)
  reference.
</Aside>
