...
BooleanColumn::new('active', 'Status')
    ...
    // Optionnel: URL custom, sinon /_ux-datatables/boolean/toggle
    ->setToggleAjax('/admin/user/toggle-active', 'id', 'PATCH');
```

Le switch envoie une requête AJAX avec `entity`, `id`, `field` et `value` pour mettre à jour la propriété booléenne.

Si votre table utilise `#[AsDataTable(Entity::class)]`, `entity` est injecté automatiquement dans le `BooleanColumn`.
Sinon, vous pouvez le définir manuellement:
...
Le contrôleur fourni par le bundle écoute `POST|PATCH /_ux-datatables/boolean/toggle`.
La gestion CSRF peut être ajoutée côté application (endpoint custom) si nécessaire.
