...
The bundle supports 8 DataTables CSS frameworks out of the box. The Stimulus controller **automatically detects** which stylesheet you have loaded and picks the matching JavaScript integration — no PHP configuration required.
...
<Aside type="note" title="DataTables 3 theming">
  DataTables 3 no longer supports the old SCSS variable customization path. Keep framework CSS
  packages aligned with the detected theme and put overrides in your application's CSS.
</Aside>
