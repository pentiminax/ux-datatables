...
public function configureDataTable(DataTable $table): DataTable
{
    return $table
        ...
        ->select(style: SelectStyle::MULTI)
        ->columnControl();
}
```
...
<Table
  ...
  rows={[
    ...
    ['`configureDataTable(DataTable $table)`', 'Configure table options and extensions'],
...
<Table
  ...
  rows={[
    ...
    [
      '`projectPage(array $items)`',
      'Batch-transform an already-paginated page of source entities. An export is batched instead of paged — see [Custom Exporters](/reference/custom-exporters/#projectpage-and-batching)',
    ],
    [
      '`createSearchStrategyRegistry()`',
      'Register custom per-column search logic — see [Search Strategies](/reference/search-strategies/)',
    ],
    [
      '`createSearchPredicateBuilder()`',
      'Replace how global search builds a column condition — see [Search Strategies](/reference/search-strategies/#global-search)',
    ],
  ]}
/>
...
<Aside type="note">
  `getDataTable()` is the rendering entrypoint; `prepareForRendering()` stays idempotent and does
...
- [Web Profiler Panel](/ux-datatables/reference/profiler/)
