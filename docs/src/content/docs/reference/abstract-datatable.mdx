...
The `$translator` property is automatically injected:

```php
public function configureColumns(): iterable
{
    yield TextColumn::new('name')
        ->setTitle($this->translator->trans('users.columns.name'));

    yield TextColumn::new('email')
        ->setTitle($this->translator->trans('users.columns.email'));
}
```
...
<Aside type="tip">
  Use the [Maker Command](/reference/maker/) to generate `AbstractDataTable` classes from Doctrine entities.
</Aside>
