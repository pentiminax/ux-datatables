...
2. The Stimulus controller fetches a pre-filled form from `POST /datatables/ajax/edit-form/view`
...
The Stimulus controller detects the CSS framework on page load. When no Bootstrap stylesheet is found, it falls back to `dt` and uses the built-in `DialogModalAdapter` with the default `@PentiminaxDataTables/modal/datatables/` Twig template.
...
data_tables:
  edit_modal:
    template: '@PentiminaxDataTables/modal/bs5/edit_modal.html.twig'
    body_template: '@PentiminaxDataTables/modal/bs5/_form_body.html.twig'
```
...
data_tables:
  edit_modal:
    template: '@PentiminaxDataTables/modal/datatables/edit_modal.html.twig'
    body_template: '@PentiminaxDataTables/modal/datatables/_form_body.html.twig'
    default_title: 'Edit'
```
