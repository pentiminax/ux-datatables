...
final class CaseInsensitiveStartsSearchStrategy implements SearchStrategyInterface
{
    public function apply(
        ...
        // getSearchField() is the column's setSearchField() override; fall back to the
        // display field so a column that does not set one behaves as before.
        ...
        // Apply the joins the column declared with addSearchJoin(). Idempotent.
...
`DefaultSearchPredicateBuilder` consults the column's own
...
`$field` is resolved by the caller from `getSearchField()` before the builder is reached, so a
`setSearchField()` override and any `addSearchJoin()` are already in effect. A column returning
`null` from `buildSearchPredicate()` has no opinion, not "skip me": the type dispatch runs.
