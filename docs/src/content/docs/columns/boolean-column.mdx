...
description: Render boolean values as toggle switches with optional AJAX update support
---

# Boolean Column

`BooleanColumn` renders a `true`/`false` (or `1`/`0`) field as text by default. When combined with `renderAsSwitch()`, the cell becomes an interactive toggle switch and toggling it sends an AJAX request to update the field server-side.

Switch markup is produced by a frontend [column style adapter](/ux-datatables/guide/styling/#column-chrome-badges--switches): a Bootstrap 5 form switch when a Bootstrap DataTables stylesheet is detected, or a Tailwind toggle otherwise (including the default `dt` theme).
...
`renderAsSwitch()` turns the cell into a live switch. Clicking it sends an AJAX `PATCH` request to update the field.
...
<Table
  ...
  rows={[
    ...
    ['`renderAsSwitch(bool $defaultState = false)`', 'Enable switch rendering with AJAX toggle'],
