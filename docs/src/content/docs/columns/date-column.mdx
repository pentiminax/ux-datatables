...
so it needs no extra dependency, follows the locale of the `<html lang>` attribute, and stays
correct in a tab left open. Sorting and searching keep using the underlying date, and
