...
<Table
  ...
  rows={[
    ...
    ["``Action::new($name, $label = '', $className = '')``", 'Create a custom link action identified by a unique `$name`. Unlike the built-in factories (one per type), several custom actions coexist as long as their names differ. Provide the destination with `linkToUrl()`'],
  ]}
/>
...
The built-in factories store **one action per type**, so adding two `Action::detail()` would
overwrite the first. To render several independent link actions in the same column, use
...
provide, so any number of them can coexist as long as the names differ.
