...
is evaluated as-is, and an action with no configured permission falls back to
`Permission::DT_VIEW_ROW_DETAILS` on the entity. An action the user cannot see is an action they
cannot fetch.
