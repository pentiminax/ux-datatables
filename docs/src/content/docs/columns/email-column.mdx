...
`renderAsText()` renders the email as plain escaped text without a `mailto:` link. `mask()` and `setDisplayValue()` still apply to the displayed text. `obfuscate()` has no effect in this mode because there is no `mailto:` href.
...
<Table
  ...
  rows={[
    ...
    [
      '`renderAsText(bool $renderAsText = true)`',
