...
`#[DataTableColumn(options: ['exportable' => false])]`). Action columns, template columns, and
hidden columns (`setVisible(false)`) are already excluded. Order in the file is `configureColumns()`
order, and headers are column titles.
...
- Adding `ButtonsExtension` without adding `Feature::BUTTONS` to the layout — `buttons()`
  avoids this entirely.
- Calling `buttons()` twice and expecting the lists to merge; the second call replaces the first.
