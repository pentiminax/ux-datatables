...
`configureBulkActions()`; the bundle adds the checkbox selection, the contextual bar above the
table, and the endpoint that executes the handler.
...
<Table
  ...
  rows={[
    ...
    ['``handler($handler)``', 'The closure that receives `BulkRecords` and `BulkActionContext`'],
  ]}
/>
...
<Table
  ...
  rows={[
    ...
    ["``setIdField($field)``", 'Row key written as `DT_RowId` and sent back as the selection (default `id`)'],
    ["``position($layoutPosition)``", 'DataTables layout cell for the bar (default `topStart`)'],
  ]}
/>
...
`DoctrineDataProvider` does. Any other provider answers `400` for a select-all, and so does a table
...
The bar's chrome is translated server-side from the `DataTables` catalog: `bulk.bar.selected`,
`bulk.bar.selectAllMatching`, `bulk.bar.allMatchingSelected`, `bulk.bar.clear`, `bulk.bar.confirm`,
`bulk.bar.cancel`, `bulk.bar.processed` and `bulk.bar.skipped`. The count-bearing ones accept a
`{count}` placeholder.
